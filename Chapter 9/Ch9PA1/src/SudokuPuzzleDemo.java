...
public class SudokuPuzzleDemo {
	...
	public static void main(String[] args) {
		...
		while(!done)
		{
			...
			{
				System.out.println("Which row (1-25) and column (1-25) do you want to change?");
				int row = reader.nextInt() - 1; 
				int col = reader.nextInt() - 1;
				System.out.println("What should the value (1-25) be?");
				int value = reader.nextInt();
				puzzle.addGuess(row, col, value);
			}
			if(response.equals("g"))
			{
				System.out.println("Which row (1-25) and column (1-25) do you want to get values for?");
				int row = reader.nextInt() - 1;
				int col = reader.nextInt() - 1;
